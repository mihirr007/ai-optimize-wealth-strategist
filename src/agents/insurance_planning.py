...
class InsurancePlanningSignal(BaseModel):
...
def insurance_planning_agent(state: WealthAgentState, agent_id: str = "insurance_planning_agent"):
    """Analyzes Insurance Planning opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Insurance Planning opportunities")
    ...
    signal = InsurancePlanningSignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Insurance Planning analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Insurance Planning strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "InsurancePlanning Agent")
