...
class DebtStrategySignal(BaseModel):
...
def debt_strategy_agent(state: WealthAgentState, agent_id: str = "debt_strategy_agent"):
    """Analyzes Debt Strategy opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Debt Strategy opportunities")
    ...
    signal = DebtStrategySignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Debt Strategy analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Debt Strategy strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "DebtStrategy Agent")
