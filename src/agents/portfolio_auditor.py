...
class PortfolioAuditorSignal(BaseModel):
...
def portfolio_auditor_agent(state: WealthAgentState, agent_id: str = "portfolio_auditor_agent"):
    """Analyzes Portfolio Auditor opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Portfolio Auditor opportunities")
    ...
    signal = PortfolioAuditorSignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Portfolio Auditor analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Portfolio Auditor strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "PortfolioAuditor Agent")
