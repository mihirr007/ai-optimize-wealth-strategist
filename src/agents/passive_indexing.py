...
class PassiveIndexingSignal(BaseModel):
...
def passive_indexing_agent(state: WealthAgentState, agent_id: str = "passive_indexing_agent"):
    """Analyzes Passive Indexing opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Passive Indexing opportunities")
    ...
    if symbols:
        ...
        print(f"📊 [{agent_id.upper()}] No symbols found for market data analysis")

    # Enhanced analysis with market data
    signal = PassiveIndexingSignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Passive Indexing analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Passive Indexing strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "PassiveIndexing Agent")

    progress.update_status(agent_id, client_profile.client_id, "Done")

    # Show current state of agent signals
    from graph.state import show_current_agent_signals
    show_current_agent_signals({
        "data": {
            **state["data"],
