...
class EstatePlanningSignal(BaseModel):
...
def estate_planning_agent(state: WealthAgentState, agent_id: str = "estate_planning_agent"):
    """Analyzes Estate Planning opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Estate Planning opportunities")
    ...
    signal = EstatePlanningSignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Estate Planning analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Estate Planning strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "EstatePlanning Agent")
