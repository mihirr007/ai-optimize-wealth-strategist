...
class FactorInvestingSignal(BaseModel):
...
def factor_investing_agent(state: WealthAgentState, agent_id: str = "factor_investing_agent"):
    """Analyzes Factor Investing opportunities with real-time market data"""
    data = state["data"]
    client_profile = data["client_profile"]
    portfolio = data["portfolio"]

    progress.update_status(agent_id, client_profile.client_id, "Analyzing Factor Investing opportunities")
    ...
    signal = FactorInvestingSignal(
        signal="maintain",
        confidence=70.0,
        reasoning=f"Factor Investing analysis with real-time market data for {len(symbols)} symbols",
        recommendations=["Consider Factor Investing strategies", "Review current allocation"],
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(signal.model_dump(), "FactorInvesting Agent")
